...
function selectWinner(){ //if the one of following winning combination match then select the winner
    if(checkIdSign(1,2,3,playerSign) || checkIdSign(4,5,6, playerSign) || checkIdSign(7,8,9, playerSign) || 
        ...
        wonText.innerHTML = `Player <p>${playerSign}</p> Pemenangnya!`; //displaying winning text with passing 
                                                                         //playerSign (X or O)
        ...
        if(getIdVal(1) != "" && getIdVal(2) != "" && getIdVal(3) != "" && getIdVal(4) != "" && getIdVal(5) != "" && 
            ...
            wonText.textContent = "Pertandingan Seimbang"; //displaying draw match text
        }
    }
}
